...
local function getAvatarMaterial(steamid64, callback)
    ...
    local fallback
	if RoachHook.Detour.os.time() - file.Time("avatars/" .. steamid64 .. ".png", "DATA") > AVATAR_IMAGE_CACHE_EXPIRES then
		fallback = RoachHook.Detour.Material("../data/avatars/" .. steamid64 .. ".png", "smooth")
	elseif RoachHook.Detour.os.time() - file.Exists("avatars/" .. steamid64 .. ".jpg", "DATA") > AVATAR_IMAGE_CACHE_EXPIRES then
		fallback = RoachHook.Detour.Material("../data/avatars/" .. steamid64 .. ".jpg", "smooth")
	end

	-- If a fallback couldn't be found in data/, default to vgui/avatar_default
	if not fallback or fallback:IsError() then
		fallback = RoachHook.Detour.Material("vgui/avatar_default")
	else
	...
	RoachHook.Detour.http.Fetch("https://steamcommunity.com/profiles/" .. steamid64 .. "?xml=1",
		...
		function(body, size, headers, code)
			...
			RoachHook.Detour.http.Fetch(url .. fileType,
				
				function(body, size, headers, code)
					if size == 0 or code < 200 or code > 299 then return callback(fallback, steamid64) end

					local cachePath = "avatars/" .. steamid64 .. "." .. fileType
					RoachHook.Detour.file.CreateDir("avatars")
					RoachHook.Detour.file.Write(cachePath, body) -- Write the avatar to data/

					local material = RoachHook.Detour.Material("../data/" .. cachePath, "smooth") -- Load the avatar from data/ as a 
					if material:IsError() then
						-- If the material errors, the image must be corrupt, so we'll delete this from data/ and return the fallback.
						RoachHook.Detour.file.Delete(cachePath)
...
local function clearCachedAvatars()
	for _, f in ipairs( ( file.Find("avatars/*", "DATA") ) ) do
		RoachHook.Detour.file.Delete("avatars/" .. f)
	end

	RoachHook.Detour.hook.Remove("InitPostEntity", "clearCachedAvatars") -- Just to be safe.
end
RoachHook.Detour.hook.Add("InitPostEntity", "clearCachedAvatars", clearCachedAvatars)
